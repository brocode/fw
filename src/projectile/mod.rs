...
pub fn projectile(maybe_config: Result<Config, AppError>, logger: &Logger) -> Result<(), AppError> {
  ...
  let projects_names: Vec<String> = config.projects.into_iter().map(|(_, p)| p.name).collect();
  let mut projectile_bookmarks = try!(env::home_dir().ok_or(AppError::UserError("$HOME not set".to_owned())));
...
fn persist<W>(logger: &Logger, writer: W, workspace: String, names: Vec<String>) -> Result<(), AppError> where W: io::Write, {
...
fn test_persists_projectile_config() {
  ...
  let logger = Logger::root(LevelFilter::new(slog_term::StreamerBuilder::new().stdout().build(), Level::Info).fuse(), o!());
  let names = vec!["test".to_owned(), "other".to_owned()];

  persist(&logger, &mut buffer, "/home/mriehl".to_owned(), names).unwrap();

  assert_eq!(str::from_utf8(buffer.get_ref()).unwrap(), "(\"/home/mriehl/test\" \"/home/mriehl/other\" )");
}
