...
fn logger_from_verbosity(verbosity: &u64) -> Logger {
  ...
  let drain = slog_term::StreamerBuilder::new().auto_color().stdout().build();
...
fn main() {
  ...
  let subcommand_name =
    matches.subcommand_name().expect("subcommand required by clap.rs").to_owned();
  ...
  let result: Result<String, AppError> = match subcommand_name.as_ref() {
      ...
      _ => Result::Err(AppError::InternalError("Command not known")),
    }
