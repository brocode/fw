...
impl GithubApi {
  ...
  fn page_repositories(&mut self, org: &str, after: Option<String>) -> Result<PageResult, AppError> {
    ...
    let (_, status, json) = self.client.query::<Value>(
        &Query::new_raw("query {organization(login: \\\"".to_owned() + org + "\\\"){repositories(first: 100" + &after_refinement + ") {nodes {name} pageInfo {endCursor hasNextPage}}}}")
    )?;
    if ! status.is_success() {
      Err(AppError::RuntimeError(format!("GitHub repository query failed for {}, got status {} with json {:?}", org, status, json)))
    } else {
      ...
      let nodes_json: Vec<Value> = nodes_json_value
        .as_array()
        .ok_or(AppError::InternalError("nodes in repository json is not an array"))?.to_owned();
